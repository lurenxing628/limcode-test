...
发送可选择排队到目标本轮结束：目标正在运行时，消息锚定当前 Turn 且不注入该 Turn；该 Turn 结束后，followup 由持久唤醒开启恰好一轮新 Turn，message 随目标下一轮带入。跨对话 followup 还要求整轮独占，见下文「跨对话协作」。排队等待期间不做周期轮询，不拥有目标对话的宿主也一样：目标轮次结束的提交在本宿主直接触发扫描，其他宿主的提交或该轮的恢复通过外部数据版本变化触发扫描。完成后自动回送的结果不排队，仍在请求方运行中的 Turn 安全边界注入；若用户在该轮吸收前停止它，这条结果与其他投向该轮但还没被吸收的 message 一样随下一轮带入：该轮的结束提交在同一事务里把它们改回等待下一轮（读取之后才送达的，提交会失败并重新读取），所以紧接着开始的下一轮一定能读到，不会与唤醒扫描竞争而漏到再下一轮；唤醒随即结清，不再为它重试或让对话一直显示忙碌。
