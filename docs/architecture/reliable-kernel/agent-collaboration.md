...
消息正文保存到 CAS；消息、来源、目标、回复关系、任务请求和请求对应 Turn 分别持久化。

followup 的全部投递都失败时（例如排队期间目标对话被删除，或续跑轮次始终无法建立权限），任务从未开始。收敛时沿完成回复的同一路径给请求方发送一条「Task could not start: …」回复，再把请求标为失败，请求方不会一直等待。这条回复没有来源轮次，来源对话可能已经删除；每个请求最多回复一次。
...
- `send_conversation_message`：`followup` 或 `message`，固定排队到目标本轮结束；followup 完成后的回复自动回到发送方。
  ...
  - 团队内（同根）的续派语义不变。
- `create_conversation`：以工具调用派生稳定对话身份并领取运行归属。写入任何东西之前，先检查开关、发送方仍是活动轮次中未结束的工具调用、每轮新建与分支上限和自动续派预算；然后只在空位中写入发送方冻结的工作环境和模型配置，最后在同一个事务里建立对话、主 Agent 关联、项目关联并提交 followup 任务。被拒绝或中途中断时不会留下空对话；中断后重放只补齐缺少的步骤。首个任务就是持久的创建标记：对话被用户删除后，重放同一调用会被拒绝，不会重新建出空对话。不写 `ConversationOriginLink`。
