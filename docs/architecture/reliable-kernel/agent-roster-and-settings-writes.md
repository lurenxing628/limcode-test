...
已有 AnswerBridge 是实际续聊标识，A1/A2 是其模型可见别名。普通请求与压缩请求共用持久 recipe 的映射恢复入口；压缩、成员暂时退出清单和 Host 重启都不能把旧编号分配给新人。保留历史别名只保证引用可辨认，不授予操作权限。Fork 的历史请求可能引用源会话的成员，但 Fork 不继承原会话的 ChildExecution 父关系；查询和操作均须按当前会话的真实父链重新验证。
