...
确认前，确认框用只读的 `compression.rebuildPreview.get` 按重建时相同的维护 authority、当前压缩设置、来源展开和冻结执行计划估算：原始记录的本地 token 估算、压缩模型窗口、预计请求数（分段数与合并数），以及超出 32 段、超出重建上限或当前方式不能分段时明确不可重建并禁用确认。预估不发请求、不写数据库或 CAS。分段摘要的内核预检只要求固定开销和旧摘要放得进窗口，超长单回合交给 Provider 侧切分器按每次请求检查。
