...
- MCP 工具只按冻结的 `sourceConfigs` 准入（`toolAllowedByPolicy`），工具提供、派发准入、MCP policy gate 和设置页共用同一规则。内置只读 Agent（Explore、Reviewer）和只读工作流（审查、只读）携带全来源拒绝 `'*'`：其它层开启的 MCP 服务不会带给它们，在这些范围保存没有列表的记录时仍然保留；更具体的层和列表都不能重新开启，只有在该 Agent 或工作流自己的工具设置里开启某个服务才会放行。这些范围自己保存的 `'*'` 值（包括 `{ enabled: true }`）不会覆盖内置的全来源拒绝，只有写明真实来源 id 的设置才算开启；任何范围保存的 `'*'` 不是对象时（如 `null`、字符串、数组）按全来源拒绝处理。
