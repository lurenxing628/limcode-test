...
- 存储的 `allowedTools` 只能缺省或是工具名数组；`null`、字符串等手工写入的值使编译失败，设置页按“没有工具可用”显示，不当作“不收窄”。
- 全局“继承默认”只去掉全局的工具列表，保留逐工具配置（审批、显示、跨对话开关、协作上限）、MCP 来源设置和预设；记录不再有内容时删除。
- 在没有列表的范围里第一次改工具开关时，从后端当前实际允许的工具开始；全局和工作流还并入每个有自己列表或内置列表的 Agent 当前在该范围得到的工具，避免悄悄收走它们。已有列表的范围按保存的列表编辑，被上层挡住的条目也保留。
