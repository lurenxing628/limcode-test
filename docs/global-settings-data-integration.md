...
- 有列表的层是上限，没有列表的层不收窄。整条链（全局、Agent 已保存或内置列表、工作流已保存或内置列表、对话）都没有列表时，以默认工具集为底：非 MCP 且 `defaultEnabled !== false` 的工具（`defaultToolNames`）。因此自定义 Agent 在全局没有列表时拿到默认工具集，内置主 Agent 仍保留自己列表里的 `transfer` 等默认关闭的工具。
- 存储的 `allowedTools` 只能缺省或是工具名数组；`null`、字符串等手工写入的值使编译失败，不当作“不收窄”，内置 Agent/工作流范围也不会因此改用内置列表。设置页在该范围和它下面的范围显示“工具列表无效”并指出要重置的层；重置（全局用“继承默认”，其它范围用“恢复继承”）前不能修改工具开关、跨对话开关，也不会有任何编辑把这个值改写成别的列表。
- 全局“继承默认”只去掉全局的工具列表，保留逐工具配置（审批、显示、跨对话开关、协作上限）、MCP 来源设置和预设；记录不再有内容时删除。
...
- MCP 工具按冻结的 `sourceConfigs` 准入（`toolAllowedByPolicy`）：已开启的来源准入它没有停用的工具，关闭或被拒绝的来源一律拒绝；没有任何层配置的来源才回落到工具列表，名字在有效列表里就准入。工具提供、派发准入、Provider 适配器、token 估算、MCP policy gate 和设置页共用同一规则。内置只读 Agent（Explore、Reviewer）和只读工作流（审查、只读）携带全来源拒绝 `'*'`：其它层开启的 MCP 服务不会带给它们，在这些范围保存没有列表的记录时仍然保留；更具体的层和列表都不能重新开启，只有在该 Agent 或工作流自己的工具设置里开启某个服务才会放行。这些范围自己保存的 `'*'` 值（包括 `{ enabled: true }`）不会覆盖内置的全来源拒绝，只有写明真实来源 id 的设置才算开启；任何范围保存的 `'*'` 不是对象时（如 `null`、字符串、数组）按全来源拒绝处理。
- 设置页的 MCP 开关只改它显示的那一项：勾选或取消一个 MCP 工具（MCP 页、工具列表或服务下的工具）只写当前范围对该来源的设置，同时写明该来源其它工具现在的开关状态（上层已停用的工具留给上层），不会开启整个服务，也不会顺带新建工具列表。被上层关闭的服务或工具不可勾选；内置只读 Agent 或工作流下的对话里，MCP 服务开关不可用，并提示到该 Agent 或工作流自己的工具设置里开启。
- MCP 工具名是 `服务名_原始工具名`，来源设置的 `disabledTools` 和工具列表都按这个名字保存。服务名先转成小写 ASCII（非 `[a-z0-9_-]` 的字符换成 `-`）；转换后为空时（如纯中文服务名）改用服务的稳定 id，普通 ASCII 服务名的工具名不变。仍然重名（与内置工具或彼此）时按来源 id 的固定顺序追加 `_2`、`_3`，与服务连上的先后无关，所以保存的名字不会换到另一个服务的工具上。
