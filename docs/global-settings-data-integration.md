...
- 给模型看的 MCP 工具名是 `服务名_原始工具名`。服务名先转成小写 ASCII（非 `[a-z0-9_-]` 的字符换成 `-`）；转换后为空时（如纯中文服务名）改用 `mcp-<服务 id 的 SHA-256 前 8 位>`，普通 ASCII 服务名的工具名不变。整名超过 64 个字符（OpenAI 和 Gemini 的函数名上限）时截到 55 个字符并接上 `_<整名的 SHA-256 前 8 位>`。仍然重名（与内置工具或彼此）时按来源 id 的固定顺序追加 `_2`、`_3`（同样不超过 64 个字符），与服务连上的先后无关。这些名字只由服务 id、服务名和原始工具名决定，每次启动都一样。
