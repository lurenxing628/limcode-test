...
OpenAI Compatible 只代表编码格式。自定义网关、未识别模型和未来模型不自动获得 OpenAI 思考档位或压缩能力。Google 官方兼容入口与 Gemini 原生入口使用不同参数编译路径。DeepSeek 等未确认精确能力的模型使用 Provider 默认或明确标记的高级未验证设置，不猜测所有模型都接受同一组等级。
