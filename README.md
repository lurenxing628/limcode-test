...
同一数据目录固定使用一套当前历史库，增减工作区文件夹不会切换历史。首次发现多套旧库时会要求选择；其它库可通过“历史与存储管理”只读分页查看。切库需要关闭使用同一数据目录的其它窗口，并重载当前窗口。

从已发布的 Runtime epoch 3 或 4 升到 epoch 5 时，扩展会先核对旧库并持久备份 SQLite，再离线升级原库；原有对话、消息、附件、CAS、配置和工作区保留。新版也会先收敛旧版已中断的 3→4 升级，再继续到 epoch 5。升级中断会在下次启动时按日志继续；旧库结构不符合已发布定义时会保留原数据并报告错误，不会自动启用空库。
...
“历史与存储管理”也提供按需空间统计、非当前整库删除和归档重置。CAS 保存历史正文与附件；归档保留完整备份，本身不会释放这些空间。工作目录按会话显式选择、继承、绑定项目、保存的默认值和唯一候选确定，失效时会提示重新选择；已开始的回合保留原目录。
