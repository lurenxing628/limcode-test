...
async function fixture(run, notify) {
  ...
  let command = 0;
  const user = (conversationId, args, commandId = `command-${++command}`) => board.executeUser({ conversationId, commandId }, args);
  try {
    ...
    await database.transaction(['root', 'sibling'].map(id => repo('Turn').insert({ id: `${id}-turn`, conversation_id: id, status: 'active', created_at: now, updated_at: now, terminal_at: null })));
    ...
    await run({ get database() { return database; }, get board() { return board; }, store, rows, user,
      ...
      async tool(id = 'board-call', conversationId = 'root') {
...
test('board derives sibling scope, preserves CAS text across reopen and rejects unrelated conversations', async () => {
  await fixture(async f => {
    const { channel } = await f.user('root', { operation: 'create_channel', name: 'Research' });
    assert.equal(channel.name, 'research');
    assert.equal(typeof channel.createdAt, 'number');
    const post = await f.user('sibling', { operation: 'post', channelId: channel.id, text: 'Shared investigation 中文 🛰️' });
    await assert.rejects(f.user('outsider', { operation: 'read_post', postId: post.postId }), /task tree/);
    await assert.rejects(f.user('outsider', { operation: 'post', channelId: channel.id, text: 'inject' }), /task tree/);
    await assert.rejects(f.user('root', { operation: 'post', channelId: channel.id, text: 'notify external', notifyConversationIds: ['outsider'] }), /outside/);
    await f.reopen();
    const read = await f.user('root', { operation: 'read_post', postId: post.postId });
    assert.equal(read.text, 'Shared investigation 中文 🛰️');
    assert.equal(read.post.authorKind, 'user');
    assert.equal(read.post.authorConversationId, 'sibling');
    assert.equal((await f.rows('CollaborationBoardPost')).length, 1);
    assert.equal((await f.rows('CollaborationBoardPostSourceLink'))[0].source_tool_call_id, null);
  });
});

test('tool provenance is verified; exactly-once command retry cannot change body or resurrect subscriptions', async () => {
  await fixture(async f => {
    const { channel } = await f.user('root', { operation: 'create_channel', name: 'general' });
    const source = await f.tool();
    ...
    assert.equal((await f.rows('CollaborationBoardPost')).length, 1);
    assert.equal((await f.user('root', { operation: 'read_post', postId: one.postId })).post.authorKind, 'tool');
    ...
    await f.user('sibling', subscribe, 'subscribe-once');
    await f.user('sibling', { operation: 'unsubscribe', channelId: channel.id });
    assert.equal((await f.user('sibling', subscribe, 'subscribe-once')).deduplicated, true);
    assert.equal((await f.user('sibling', { operation: 'list_channels' })).channels[0].subscribed, false);
  });
});
...
test('root and reply subscriptions notify running members only; failure preserves posts and retry never replays notices', async () => {
  const notices = [];
  await fixture(async f => {
    const { channel } = await f.user('root', { operation: 'create_channel', name: 'notices' });
    for (const conversation of ['sibling', 'idle']) await f.user(conversation, { operation: 'subscribe', channelId: channel.id });
    const args = { operation: 'post', channelId: channel.id, text: 'root discussion' };
    const post = await f.user('root', args, 'notice-post');
    assert.deepEqual(new Set(post.notifications.map(item => item.status)), new Set(['failed', 'skipped_idle']));
    assert.equal(notices.length, 1);
    const retry = await f.user('root', args, 'notice-post');
    assert.equal(retry.notificationReplay, 'not_replayed');
    assert.equal(notices.length, 1);
    assert.equal((await f.user('sibling', { operation: 'read_post', postId: post.postId })).text, 'root discussion');
    const reply = await f.user('sibling', { operation: 'post', threadId: post.threadId, text: 'reply' });
    ...
    await assert.rejects(f.user('root', { operation: 'post', threadId: reply.postId, text: 'nested' }), /discussion root/);
    assert.equal((await f.user('sibling', { operation: 'read_thread', threadId: post.threadId })).subscribed, true);
...
test('bounded Unicode reads, case-insensitive search, scoped cursors and complete reply pagination', async () => {
  await fixture(async f => {
    const { channel } = await f.user('root', { operation: 'create_channel', name: 'paging' });
    const text = '😀中文AbC'.repeat(400);
    const root = await f.user('root', { operation: 'post', channelId: channel.id, text });
    let offset = 0; let full = '';
    do {
      const page = await f.user('sibling', { operation: 'read_post', postId: root.postId, offsetChars: offset, limitChars: 101 });
      full += page.text; offset = page.nextOffsetChars;
    } while (offset !== undefined);
    assert.equal(full, text);
    for (let i = 0; i < 5; i++) await f.user('sibling', { operation: 'post', threadId: root.threadId, text: `reply ${i}` });
    const ids = []; const previews = []; let cursor;
    do {
      const page = await f.user('root', { operation: 'read_thread', threadId: root.threadId, limit: 2, ...(cursor ? { cursor } : {}) });
    ...
    const search = await f.user('root', { operation: 'search', channelId: channel.id, query: 'abc' });
    assert.equal(search.posts.length, 1);
    assert.equal(search.posts[0].id, root.postId);
    const page = await f.user('root', { operation: 'search', channelId: channel.id, limit: 1 });
    await assert.rejects(f.user('root', { operation: 'search', channelId: channel.id, query: 'reply', cursor: page.nextCursor }), /cursor/);
    await assert.rejects(f.user('root', { operation: 'read_post', postId: root.postId, limitChars: 20001 }), /limitChars/);
    assert.equal((await f.user('root', { operation: 'list_threads', channelId: channel.id })).posts.length, 1);
  });
});
...
test('concurrent channel creation is one shared channel and strict arguments reject ignored mutations', async () => {
  await fixture(async f => {
    const [first, second] = await Promise.all([
      f.user('root', { operation: 'create_channel', name: 'Common', subscribe: false }),
      f.user('sibling', { operation: 'create_channel', name: 'COMMON', subscribe: false })
    ]);
    ...
    await assert.rejects(f.user('root', { operation: 'list_channels', query: 12 }), /query must be text/);
    await assert.rejects(f.user('root', { operation: 'list_threads', channelId: first.channel.id, text: 'silently mutate' }), /Unexpected/);
    await assert.rejects(f.user('root', { operation: 'list_channels', cursor: Buffer.from('null').toString('base64url') }), /cursor/);
  });
});
