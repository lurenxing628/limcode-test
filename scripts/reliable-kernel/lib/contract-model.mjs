...
function validateTool(tool, failures) {
  ...
  for (const marker of ["全来源拒绝'*'", '保存无列表记录时仍保留', '只有该Agent或工作流自己的sourceConfigs能开启', 'toolAllowedByPolicy']) {
...
function validateContext(context, failures) {
  ...
  for (const marker of ['只复制已终止轮次', 'ConversationForkRejectedError', '本 ToolCall 自己的来源行', '整轮可见消息', '全部 ModelRequest', '自己的 AuthoritySnapshot', '子 Agent 自己的轮次仍按其任务编译权限', 'ModelContextProjection 重新挂到分支', 'compression.forkOwnership', '只保留先于切点的压缩', '改用压缩前的根', '可见消息全被删除或重试丢弃', '恰好含其保留部分的根', '切点永不延长', '追加在切点之后', '只检查分支自身的片段', '不参与 fork 身份与重放比较']) {
...
function validateSubagent(subagent, failures) {
  ...
  if (crossConversation?.switch !== 'ToolPolicy.toolConfigs.run_agent.config.crossConversationCollaboration; boolean; default-off; non-boolean-fails-closed; frozen-in-Turn-authority; configSchema-defaultValue-only'
    ...
    || crossConversation?.allowlist !== 'enabling-the-switch-extends-only-a-scope-own-saved-allowedTools-with-the-missing-tools(read-only-pair-without-run_agent)-and-records-them-in-crossConversationGrantedTools; a-scope-without-its-own-list-stores-only-the-switch-and-a-list-less-ToolPolicy-narrows-nothing; with-no-list-on-the-chain-the-default-tool-set-applies; send-create-and-fork-are-offered-and-admitted-only-while-the-Turn-effective-allowedTools-include-run_agent(otherwise-list-and-read-only); turning-off-removes-only-granted-tools; restoring-removes-them-unless-the-inherited-switch-is-still-on; an-explicit-user-enable-clears-a-tool-grant-mark; backend-never-widens-a-saved-allowlist'
    || crossConversation?.delivery !== 'running-target-queues-behind-its-current-Turn; followup-starts-its-own-Turn-and-only-that-Turn-consumes-it; followup-waits-behind-any-running-Turn-at-dispatch-including-one-started-after-its-anchor; followups-from-different-senders-start-sequential-Turns-never-merged; that-Turn-spends-only-the-requester-budget; message-joins-the-next-Turn-whatever-starts-it; completion-replies-are-not-queued'
